...
4. 三、本发明技术方案的详细阐述
   ...
   - 3.4.1 符号与公式（出现公式或形式化变量时**须设**；**先**写案件目录 `formula_plan.yaml` 再成文；材料式保真，见 **§7.7**、**`template_reference.md` §1.4.1**）
...
撰写 **3.4.1**、**3.5** 及全文含 LaTeX 的段落时须遵循；正/反例见 **`template_reference.md` §1.4.1**。  
...
代理人须能对着本案场景读懂第三章。细则与正/反例见 **`template_reference.md` §1.1 / §1.2 / 实施例 / 第五章**。

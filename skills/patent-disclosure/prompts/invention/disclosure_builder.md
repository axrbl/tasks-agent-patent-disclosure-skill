...
4. 三、本发明技术方案的详细阐述
   ...
   - 3.4.1 符号与公式（出现公式或形式化变量时**须设**；**先**写案件目录 `formula_plan.yaml` 再成文；材料式保真，见 **§7.7**、**`template_reference.md` `rules-general.md` §1.4.1**）
...
5. **时间戳后缀（凡落盘交付必遵循）**：凡写入用户产出目录、作为**向用户交付**的技术交底书 **`.md` / `.docx`**，主文件名须为：  
   **`{`rules-general.md` §4.2 规范化案件名}_{YYYYMMDDHHmmss}.md`** 与同名 **`.docx`**。  
...
仅当用户指定路径或说「这张图是场景示意，补充到交底书」：

1. **`Read`** 该图，判别种类（见 `project_scan.md`「图片与裸图目录」）。
2. 图标、切图、营销场景：**不入文**，对话说明原因。
3. 技术示意图 / 拓扑 / 对照图：拷到案件 `assets/`，在 3.1 或第六章用 `![…](assets/…)` 嵌入，写「如图 N」，给图题。不要改画，不要扫描其他素材目录。
4. 框图和流程仍用 mermaid；点名图是**额外**附图，不替代 3.2 / 3.4。
...
撰写 **3.4.1**、**3.5** 及全文含 LaTeX 的段落时须遵循；正/反例见 **`template_reference.md` `rules-general.md` §1.4.1**。  
...
**版本与覆盖**：凡交付均以 **`rules-general.md` §4.2 第 5 点**「**案件名 + 时间戳**」落盘，同目录下多文件即版本历史。**可选**：另存 `versions/` 手工快照非强制。
...
2. 承接方式：Agent **`Read`** **`iteration_context.md`**，再按 **`merger.md`** 以**当前交付稿为基准**合并，**另存**新时间戳 **`.md` / `.docx`**（`rules-general.md` §4.2 第 5 点），并维护 **`交底书修订对话记录.md`**。  
...
定稿交付回复末尾，按 **`skills/patent-exam-policy/prompts/soft_nudge.md`** 决定是否加 **至多一句**（默认低频；不入正文；不自动进入政策简报）。
...
代理人须能对着本案场景读懂第三章。细则与正/反例见 **`template_reference.md` `rules-general.md` §1.1 / `rules-general.md` §1.2 / 实施例 / 第五章**。

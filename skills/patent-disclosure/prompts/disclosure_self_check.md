...
- [ ] **参数命名**：全文统一，避免同义不同名混用（公式符号与 **`rules-general.md` §4.3 场景术语**分开查：场景词换名须整族替换，见 merger / correction）
...
- [ ] **交付文件名**：凡落盘交付的交底书是否均为 **`{案件名}_{YYYYMMDDHHmmss}.md`** 及同名 `.docx`（`rules-general.md` §4.2 第 5 点，**含首次定稿与迭代**），未无故覆盖旧稿
...
发现问题则**直接修订正文**后再交付。  
**公式类（发明）**：Step 8 须完成 `rules-general.md` §4.4 核对；有误则改公式并联动修订相关章节，**不要**只向用户报告问题而不改稿（除非缺少必要技术事实、无法从正文与材料推断正确写法）。  
**术语 / 标题贯穿 / 实施例（发明）**：`rules-general.md` §4.3 相关项不通过则改 3.1、框图、流程、实施例或第五章后再交付；换叫法须整族替换，机制不丢。

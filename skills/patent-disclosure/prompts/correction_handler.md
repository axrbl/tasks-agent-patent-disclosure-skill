...
与 `merger.md` 相同：先 **`Read`** **`skills/patent-disclosure/prompts/iteration_context.md`**，再 **Read** 当前定稿与纠正意图；纠正结果须**另存为** **`{案件名}_{YYYYMMDDHHmmss}.md`** 及同名 `.docx`（`rules-general.md` §4.2 第 5 点），**禁止**默认覆盖旧稿。**禁止**在迭代意图下无合并/纠正落盘却回到 Step 3–4 全文专利点分析（除非用户明确要求重做专利点）。
